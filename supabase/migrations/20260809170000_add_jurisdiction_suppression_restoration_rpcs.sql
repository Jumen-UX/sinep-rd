-- Administrative suppression/restoration of current jurisdiction accounts.
-- These operations update the current organigram and audit trail only.
-- Public institutional history is handled separately by documented historical events.
...
declare
  ...
  v_type_key text;
  v_parent_edge public.jurisdiction_account_edges%rowtype;
  v_parent_name text;
  v_active_children integer := 0;
...
begin
  ...
  select account.*, entity.*, type.key
  into v_account, v_entity, v_type_key
  ...
  if not v_account.is_current or v_account.status <> 'active' then
    v_errors := v_errors || jsonb_build_array('La jurisdicción ya no está activa en el organigrama vigente.');
  end if;
  ...
  if p_effective_date is null then
    ...
    v_errors := v_errors || jsonb_build_array('La fecha de supresión no puede ser anterior al inicio de vigencia de la cuenta.');
  end if;
  ...
  where edge.parent_account_id = p_account_id
    and edge.is_current
    and edge.status = 'active';

  if v_active_children > 0 then
    v_errors := v_errors || jsonb_build_array(
      format('La jurisdicción tiene %s dependencia(s) vigente(s). Deben resolverse antes de suprimirla.', v_active_children)
    );
  end if;

  select edge.*, parent_entity.name
  into v_parent_edge, v_parent_name
  from public.jurisdiction_account_edges edge
  ...
  where edge.child_account_id = p_account_id
    and edge.is_current
    and edge.status = 'active'
  order by edge.created_at desc
  limit 1;

  if v_parent_edge.id is not null
    and p_effective_date is not null
    and p_effective_date <= v_parent_edge.valid_from then
  ...
  return jsonb_build_object(
    ...
    'jurisdiction', jsonb_build_object(
      ...
      'name', v_entity.name,
      'canonical_status', v_account.canonical_status,
      'valid_from', v_account.valid_from
    ),
    'current_dependency', case when v_parent_edge.id is null then null else jsonb_build_object(
      'edge_id', v_parent_edge.id,
      'parent_account_id', v_parent_edge.parent_account_id,
      'parent_name', v_parent_name,
      'relationship_type', v_parent_edge.relationship_type,
      'valid_from', v_parent_edge.valid_from
    ) end,
    'active_children_count', v_active_children,
...
begin
  ...
  where child_account_id = p_account_id
    and is_current
    and status = 'active'
  ...
  v_preview := public.admin_preview_jurisdiction_suppression(
    p_account_id,
    p_effective_date,
    p_reason,
  ...
  v_before_account := jsonb_build_object(
    ...
    'is_current', v_account.is_current,
    'valid_from', v_account.valid_from,
    'valid_to', v_account.valid_to,
    'entity_status', v_entity.status,
    'suppressed_at', v_entity.suppressed_at
  );
  ...
  if v_current_edge.id is not null then
    ...
    v_after_edge := v_before_edge || jsonb_build_object(
  ...
  set canonical_status = 'suppressed',
  ...
  set status = 'suppressed',
      suppressed_at = p_effective_date,
      updated_at = now()
  where id = v_entity.id;

  v_after_account := v_before_account || jsonb_build_object(
    'canonical_status', 'suppressed',
    'status', 'inactive',
    'is_current', false,
    'valid_to', p_effective_date,
    'entity_status', 'suppressed',
    'suppressed_at', p_effective_date
  );

  insert into public.jurisdiction_change_operations(
    origin, status, publication_status, primary_account_id, effective_date, reason,
    source_document_id, operation_source, created_by, applied_by, created_at, applied_at, updated_at
  ) values (
    'organizational_change', 'applied', 'internal', p_account_id, p_effective_date, btrim(p_reason),
    p_source_document_id, 'admin_organigram', v_actor, v_actor, now(), now(), now()
  ...
  if v_current_edge.id is not null then
    ...
    insert into public.jurisdiction_change_effects(
      ...
      v_operation_id, v_sequence, 'edge', v_current_edge.id, 'close_dependency', v_before_edge, v_after_edge, now()
    );
  ...
  insert into public.jurisdiction_change_effects(
    ...
    v_operation_id, v_sequence, 'account', p_account_id, 'deactivate_account', v_before_account, v_after_account, now()
  );
  ...
  return jsonb_build_object(
    ...
    'closed_edge_id', v_current_edge.id,
    'preview', v_preview
  );
end;
$$;
...
begin
  ...
  if p_account_id is null or p_parent_account_id is null then
    raise exception 'La jurisdicción y su dependencia son obligatorias' using errcode = '22023';
  end if;
  ...
  if v_account.canonical_status <> 'suppressed' and v_entity.status <> 'suppressed' then
    v_warnings := v_warnings || jsonb_build_array('La cuenta no está marcada explícitamente como suprimida; se restaurará igualmente como corrección organizativa.');
  end if;
  ...
  select edge.id into v_current_edge_id
  from public.jurisdiction_account_edges edge
  where edge.child_account_id = p_account_id and edge.is_current and edge.status = 'active'
  limit 1;

  if v_current_edge_id is not null then
  ...
  if p_effective_date is null then
    ...
    v_errors := v_errors || jsonb_build_array('La fecha de restauración debe ser posterior al cierre anterior de la jurisdicción.');
  end if;
...
begin
  ...
  insert into public.jurisdiction_account_edges(
    parent_account_id, child_account_id, relationship_type, valid_from, valid_to,
    is_current, status, visibility, source_document_id, notes, created_by
  ) values (
    p_parent_account_id, p_account_id, btrim(p_relationship_type), p_effective_date, null,
    true, 'active', v_visibility, p_source_document_id, nullif(btrim(p_reason), ''), v_actor
  ...
  insert into public.jurisdiction_change_operation_accounts(operation_id, account_id, role)
  values(v_operation_id, p_parent_account_id, 'destination')
  on conflict do nothing;

  insert into public.jurisdiction_change_effects(
    operation_id, sequence, target_type, target_id, action, before_state, after_state, applied_at
  ) values
    (v_operation_id, 1, 'account', p_account_id, 'activate_account', v_before_account, v_after_account, now()),
    (v_operation_id, 2, 'edge', v_edge_id, 'create_dependency', null, v_after_edge, now());
  ...
  v_audit_id := public.admin_write_audit_log(
    ...
    jsonb_build_object(
      ...
      'effective_date', p_effective_date,
      'source_document_id', p_source_document_id,
      'before', jsonb_build_object('account', v_before_account),
      'after', jsonb_build_object('account', v_after_account, 'dependency', v_after_edge)
    )
  );
  ...
  return jsonb_build_object(
    ...
    'audit_id', v_audit_id,
    'account_id', p_account_id,
    'current_edge_id', v_edge_id,
    'preview', v_preview
  );
end;
$$;

revoke execute on function public.admin_preview_jurisdiction_suppression(uuid, date, text, uuid) from public;
revoke execute on function public.admin_preview_jurisdiction_suppression(uuid, date, text, uuid) from anon;
grant execute on function public.admin_preview_jurisdiction_suppression(uuid, date, text, uuid) to authenticated;

revoke execute on function public.admin_apply_jurisdiction_suppression(uuid, date, text, uuid, uuid) from public;
revoke execute on function public.admin_apply_jurisdiction_suppression(uuid, date, text, uuid, uuid) from anon;
grant execute on function public.admin_apply_jurisdiction_suppression(uuid, date, text, uuid, uuid) to authenticated;

revoke execute on function public.admin_preview_jurisdiction_restoration(uuid, uuid, text, date, text, uuid) from public;
revoke execute on function public.admin_preview_jurisdiction_restoration(uuid, uuid, text, date, text, uuid) from anon;
grant execute on function public.admin_preview_jurisdiction_restoration(uuid, uuid, text, date, text, uuid) to authenticated;

revoke execute on function public.admin_apply_jurisdiction_restoration(uuid, uuid, text, date, text, uuid) from public;
revoke execute on function public.admin_apply_jurisdiction_restoration(uuid, uuid, text, date, text, uuid) from anon;
