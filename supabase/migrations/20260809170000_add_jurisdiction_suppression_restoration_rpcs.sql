-- Current-organigram lifecycle operations.
-- Administrative suppression/restoration updates organization + audit only;
-- it does not create public historical events.
...
begin
  ...
  if v_children>0 then
    v_errors := v_errors || jsonb_build_array(
      format('La jurisdicción tiene %s dependencia(s) vigente(s). Deben resolverse antes de suprimirla.',v_children)
    );
  end if;

  select edge.*, parent_entity.name
  into v_edge, v_parent_name
  ...
  order by edge.created_at desc limit 1;

  if v_edge.id is not null and p_effective_date is not null and p_effective_date<=v_edge.valid_from then
...
begin
  ...
  where parent_account_id=p_account_id and is_current and status='active' for update;
...
begin
  ...
  return jsonb_build_object(
    ...
    'requires_source',coalesce(v_rule.requires_source,false),'source_document_id',p_source_document_id
  );
end;
$$;
