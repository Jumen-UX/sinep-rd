...
as $$
  select jsonb_build_object(
    'event',jsonb_build_object(
      'id',ce.id,'title',ce.title,'status',ce.status,'load_mode',ce.load_mode,'evidence_status',ce.evidence_status,
      'event_date',ce.event_date,'effective_date',ce.effective_date,'event_type_key',cet.key,'event_type_name',cet.name,'applies_to',cet.applies_to
    ),
    'actions',coalesce((
      select jsonb_agg(jsonb_build_object(
        'id',cea.id,'action_type_key',cea.action_type_key,'action_type_name',ceat.name,'description',ceat.description,
        'changes_state',ceat.changes_state,'requires_manual_review',ceat.requires_manual_review,'status',cea.status,'notes',cea.notes,
        'subject_entity_id',cea.subject_entity_id,'subject_entity_name',subject.name,
        'target_entity_id',cea.target_entity_id,'target_entity_name',target.name,
        'subject_organization_unit_id',cea.subject_organization_unit_id,'subject_organization_unit_name',subject_unit.name,
        'target_organization_unit_id',cea.target_organization_unit_id,'target_organization_unit_name',target_unit.name,
        'relationship_type_id',cea.relationship_type_id,'relationship_type_name',crt.name,'payload',cea.payload,'sort_order',cea.sort_order,'updated_at',cea.updated_at
      ) order by cea.sort_order,cea.created_at)
