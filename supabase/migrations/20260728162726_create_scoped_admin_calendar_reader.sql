...
create or replace function public.admin_list_calendar_events(
  p_from date default current_date,
  p_to date default (current_date + interval '1 year')::date,
  p_scope_entity_id uuid default null,
  p_event_type_key text default null,
...
as $$
  select *
  from app_private.rpc_definer__admin_list_calendar_events(
    p_from,
    p_to,
    p_scope_entity_id,
    p_event_type_key,
...
revoke all on function app_private.rpc_definer__admin_list_calendar_events(date, date, uuid, text, boolean, integer) from public, anon;
grant execute on function app_private.rpc_definer__admin_list_calendar_events(date, date, uuid, text, boolean, integer) to authenticated;

revoke all on function public.admin_list_calendar_events(date, date, uuid, text, boolean, integer) from public, anon;
grant execute on function public.admin_list_calendar_events(date, date, uuid, text, boolean, integer) to authenticated;
