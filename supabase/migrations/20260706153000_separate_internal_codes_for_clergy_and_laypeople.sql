create sequence if not exists public.person_clergy_code_seq;
create sequence if not exists public.person_lay_code_seq;

create or replace function public.generate_person_internal_code_for_type(p_person_type text)
returns text
language plpgsql
as $$
begin
  if p_person_type in ('bishop', 'priest', 'deacon') then
    return 'CLERO-' || lpad(nextval('public.person_clergy_code_seq')::text, 6, '0');
  end if;

  if p_person_type = 'layperson' then
    return 'LAICO-' || lpad(nextval('public.person_lay_code_seq')::text, 6, '0');
  end if;

  return 'SINEP-' || lpad(nextval('public.person_internal_code_seq')::text, 6, '0');
end;
$$;
...
begin
  ...
  v_internal_code := public.generate_person_internal_code_for_type('priest');
