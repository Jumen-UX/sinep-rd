...
begin
  ...
  select organization_chart_id, holder_cardinality, max_current_holders
  into v_default_chart_id, v_holder_cardinality, v_max_current_holders
  from public.office_configurations
  where id=v_office_configuration_id and status='active';

  if not found then
    raise exception 'El cargo configurado no existe o no está activo' using errcode='22023';
  end if;

  v_organization_chart_id := coalesce(v_organization_chart_id,v_default_chart_id);
  ...
  if v_person_id is not null and not exists (
    select 1 from public.persons where id=v_person_id and status='active'
  ) then
    raise exception 'La persona seleccionada no existe o no está activa' using errcode='22023';
  end if;

  if v_predecessor_assignment_id is not null and not exists (
    select 1 from public.position_assignments pa
    where pa.id=v_predecessor_assignment_id
      and pa.office_configuration_id=v_office_configuration_id
      and pa.organization_chart_id is not distinct from v_organization_chart_id
      and pa.organization_unit_id is not distinct from v_organization_unit_id
      and pa.ecclesiastical_entity_id is not distinct from v_ecclesiastical_entity_id
      and pa.pastoral_entity_id is not distinct from v_pastoral_entity_id
  ) then
    raise exception 'El predecesor debe pertenecer al mismo cargo y ámbito' using errcode='22023';
  end if;

  if v_successor_assignment_id is not null and not exists (
    select 1 from public.position_assignments pa
    where pa.id=v_successor_assignment_id
      and pa.office_configuration_id=v_office_configuration_id
      and pa.organization_chart_id is not distinct from v_organization_chart_id
      and pa.organization_unit_id is not distinct from v_organization_unit_id
      and pa.ecclesiastical_entity_id is not distinct from v_ecclesiastical_entity_id
    ...
    raise exception 'El sucesor debe pertenecer al mismo cargo y ámbito' using errcode='22023';
  end if;

  if v_term_start_date is null then v_term_start_date:=v_start_date; end if;
  if v_effective_date is null then v_effective_date:=coalesce(v_start_date,v_term_start_date); end if;
  ...
  if v_person_id is not null and v_is_current then
    v_eligibility:=internal.evaluate_position_assignment_eligibility(
      v_person_id,v_office_configuration_id,v_ecclesiastical_entity_id,null,false
    );
    if not coalesce((v_eligibility->>'eligible')::boolean,false) then
      raise exception '%',coalesce(v_eligibility->>'message','La persona no cumple las condiciones del cargo') using errcode='23514';
    end if;
  end if;

  if v_is_current and v_holder_cardinality='single' then
    select coalesce(array_agg(pa.id order by pa.created_at),'{}'::uuid[])
    ...
    where pa.is_current=true and pa.record_status='active'
      ...
      and pa.ecclesiastical_entity_id is not distinct from v_ecclesiastical_entity_id
      and pa.pastoral_entity_id is not distinct from v_pastoral_entity_id;
  elsif v_predecessor_assignment_id is not null then
    v_closed_assignment_ids:=array[v_predecessor_assignment_id];
  elsif v_is_current and v_close_previous and v_person_id is not null then
    select coalesce(array_agg(pa.id order by pa.created_at),'{}'::uuid[])
    into v_closed_assignment_ids
    from public.position_assignments pa
    where pa.person_id=v_person_id and pa.is_current=true and pa.record_status='active'
      and pa.office_configuration_id=v_office_configuration_id
      and pa.organization_chart_id is not distinct from v_organization_chart_id
      and pa.organization_unit_id is not distinct from v_organization_unit_id
      and pa.ecclesiastical_entity_id is not distinct from v_ecclesiastical_entity_id
  ...
  if v_successor_assignment_id is not null then
    update public.position_assignments
    set predecessor_assignment_id=v_assignment_id,updated_at=now()
  ...
  return jsonb_build_object(
    ...
    'holder_cardinality',v_holder_cardinality,
    'max_current_holders',v_max_current_holders,
