...
begin
  ...
  v_permission_key := case
    ...
    when p_target_table in ('profiles','user_role_assignments','roles','role_permissions') then 'users.manage'
    else null
  end;
...
begin
  ...
  v_permission_key := case
    when v_action = 'import.batch.prepared' or v_action like 'import.row.%' then 'imports.prepare'
    when v_action = 'import.batch.reviewed' then 'imports.review'
    when v_action like 'import.%' then 'imports.apply'
    else 'audit.create'
  end;
...
begin
  ...
  return query
  select al.id, al.user_id, p.email::text, p.full_name::text,
         al.action, al.target_table, al.target_id, al.change_request_id, al.created_at
  ...
  where app_private.current_user_is_super_or_national()
     ...
     or (al.scope_type = 'pastoral_entity' and app_private.current_user_has_scope_access(
           'pastoral_entity', al.pastoral_entity_id, al.diocese_id, al.pastoral_area_id, al.pastoral_entity_id))
     or (al.scope_type = 'pastoral_area' and app_private.current_user_has_scope_access(
           'pastoral_area', al.pastoral_area_id, al.diocese_id, al.pastoral_area_id, null))
  order by al.created_at desc
  limit greatest(1, least(coalesce(p_limit, 100), 250));
end;
$$;

revoke all on function app_private.admin_list_recent_audit_logs(integer) from public, anon;
grant execute on function app_private.admin_list_recent_audit_logs(integer) to authenticated, service_role;
...
with check (
  ...
  and app_private.current_user_has_permission('audit.create')
  and (
    app_private.current_user_is_super_or_national()
    or (scope_entity_id is not null and app_private.current_user_can_manage_entity('audit.create', scope_entity_id))
  )
);
...
using (
  ...
  or (
    app_private.current_user_has_permission('audit.view')
    and scope_type = 'pastoral_entity'
    and app_private.current_user_has_scope_access('pastoral_entity', pastoral_entity_id, diocese_id, pastoral_area_id, pastoral_entity_id)
  )
  or (
    app_private.current_user_has_permission('audit.view')
    and scope_type = 'pastoral_area'
    and app_private.current_user_has_scope_access('pastoral_area', pastoral_area_id, diocese_id, pastoral_area_id, null)
  )
);