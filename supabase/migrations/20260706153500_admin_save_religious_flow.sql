create table if not exists public.religious_profiles (
  id uuid primary key default gen_random_uuid(),
  person_id uuid not null unique references public.persons(id) on delete cascade,
  religious_life_type text not null,
  community_name text,
  province_name text,
  profession_date date,
  canonical_status text not null default 'active',
  current_service_entity_id uuid references public.ecclesiastical_entities(id) on delete set null,
  notes_private text,
  created_by uuid references auth.users(id) on delete set null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint religious_profiles_life_type_check check (religious_life_type in ('brother','sister','consecrated_lay','other')),
  constraint religious_profiles_status_check check (canonical_status in ('active','retired','transferred','deceased','unknown'))
);

alter table public.religious_profiles enable row level security;

drop policy if exists religious_profiles_admin_select on public.religious_profiles;
drop policy if exists religious_profiles_admin_all on public.religious_profiles;

create policy religious_profiles_admin_select
on public.religious_profiles
for select
to authenticated
using (public.current_user_has_admin_role());

create policy religious_profiles_admin_all
on public.religious_profiles
for all
to authenticated
using (public.current_user_has_admin_role())
...
grant select, insert, update, delete on public.religious_profiles to authenticated;
...
declare
  ...
  v_life_type text := coalesce(nullif(payload->>'religious_life_type', ''), 'other');
...
begin
  ...
  if v_life_type not in ('brother','sister','consecrated_lay','other') then
    raise exception 'Tipo de vida religiosa inválido' using errcode = '22023';
  end if;
  ...
  v_internal_code := public.generate_person_internal_code_for_type('religious');
  ...
  insert into public.religious_profiles (
    ...
    nullif(btrim(payload->>'province_name'), ''),
    nullif(payload->>'profession_date', '')::date,
    coalesce(nullif(payload->>'canonical_status', ''), 'active'),
    nullif(payload->>'current_service_entity_id', '')::uuid,
    nullif(btrim(payload->>'religious_notes'), ''),
    v_user_id
  )
  ...
  if v_office_configuration_id is not null then
    ...
    insert into public.position_assignments (
      ...
      'Asignación creada desde asistente transaccional de nuevo religioso.',
      'pending_review',
  ...
  perform public.admin_mark_missing_fields(
    ...
    'Marcado como no identificado desde el asistente transaccional de nuevo religioso.',
    v_user_id
  );
  ...
  return jsonb_build_object(
    ...
    'religious_profile_id', v_profile_id,
    'slug', v_slug,
