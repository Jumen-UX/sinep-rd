...
begin
  ...
  v_available:=v_batch.import_type in ('personas','parroquias');
  ...
  return jsonb_build_object('batch_id',v_batch_id,'status',v_batch.status,'review_status',v_decision,'reviewed_at',now(),'can_apply',v_decision='approved' and v_available,'application_rpc_available',v_available,'audit_log_id',v_audit);
end;
$$;
