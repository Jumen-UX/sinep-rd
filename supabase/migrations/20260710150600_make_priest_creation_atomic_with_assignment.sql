...
declare
  ...
  v_priest_type text := coalesce(
    ...
    case
      when nullif(btrim(payload->>'religious_institute_name'), '') is not null
        or nullif(btrim(payload->>'religious_order'), '') is not null
      then 'religious'
      else 'diocesan'
    end
  );
  v_religious_institute_name text := coalesce(
    nullif(btrim(payload->>'religious_institute_name'), ''),
    nullif(btrim(payload->>'religious_order'), '')
  );
  v_assignment_result jsonb;
  v_assignment_id uuid;
  v_closed_previous_current_count integer := 0;
begin
  ...
  if v_priest_type not in ('diocesan', 'religious') then
    raise exception 'Tipo de sacerdote no permitido' using errcode = '22023';
  end if;

  if v_priest_type = 'religious' and v_religious_institute_name is null then
    raise exception 'Debes indicar la congregación, instituto u orden del sacerdote religioso' using errcode = '22023';
  end if;
  ...
  insert into public.clergy_profiles (
    ...
    notes_private,
    priest_type,
    religious_institute_name,
    religious_province_name,
    religious_house_entity_id
  ) values (
    v_person_id,
    ...
    coalesce(nullif(btrim(payload->>'religious_order'), ''), v_religious_institute_name),
    coalesce(nullif(payload->>'canonical_status', ''), 'active'),
    nullif(btrim(payload->>'clergy_notes'), ''),
    v_priest_type,
    v_religious_institute_name,
  ...
  on conflict (person_id) do update set
    ...
    priest_type = excluded.priest_type,
  ...
  if v_office_configuration_id is not null then
    ...
    v_assignment_result := internal.admin_save_position_assignment(
      jsonb_build_object(
        ...
        'ecclesiastical_entity_id', v_assignment_entity_id,
        'title_override', nullif(btrim(payload->>'quick_title_override'), ''),
        'start_date', v_start_date,
        'term_start_date', v_start_date,
        'assignment_status', 'active',
        'selection_method', 'appointment',
        'notes_public', nullif(btrim(payload->>'quick_notes_public'), ''),
        'notes_internal', 'Asignación creada desde asistente transaccional de sacerdote.',
    ...
    v_closed_previous_current_count := coalesce((v_assignment_result->>'closed_previous_current_count')::integer, 0);
  end if;
  ...
  return jsonb_build_object(
    ...
    'closed_previous_current_count', v_closed_previous_current_count,
    'slug', v_slug,
    'internal_reference_code', v_internal_code,
    'priest_type', v_priest_type
  );
end;
$$;
