...
const publicRevalidate = await readFile('src/lib/public/revalidate.ts', 'utf8')
...
test('public detail cache remains versioned tagged revalidable and explicitly invalidated', () => {
  ...
  assert.match(publicCache, /revalidatePublicCache\('directories'\)/)
  assert.match(publicRevalidate, /directories: \[PUBLIC_CACHE_TAGS\.dashboard, PUBLIC_CACHE_TAGS\.directories\]/)
  assert.match(publicRevalidate, /for \(const tag of tags\) revalidateTag\(tag\)/)
  assert.match(publicRevalidate, /\['\/personas', 'page'\]/)
  assert.match(publicRevalidate, /\['\/diocesis', 'page'\]/)
...
test('sprint 8 records completed performance search observability documentation and validation', () => {
  for (const item of ['S8-01', 'S8-02', 'S8-03', 'S8-04', 'S8-05', 'S8-06', 'S8-07', 'S8-08', 'S8-09', 'S8-10']) {
