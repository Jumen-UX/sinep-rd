...
test('territorial filters remain searchable and keyboard accessible', async () => {
  ...
  assert.match(component, /if \(!open\) setQuery\(selectedOption\?\.label \?\? ''\)/)
  assert.match(component, /if \(filteredOptions\.length === 0\) return/)
  assert.match(component, /scrollIntoView\(\{ block: 'nearest' \}\)/)
  assert.match(component, /rootRef\.current\?\.contains\(event\.relatedTarget/)
