...
const migrationPaths = [
  ...
  new URL('../supabase/migrations/20260714022000_finalize_organization_unit_security_contracts.sql', import.meta.url),
]
...
test('existing audit writes receive permission and scope automatically', async () => {
  const sql = await readSecurityMigrations()
...
test('person wizards route through the canonical public contract', async () => {
  const sql = await readSecurityMigrations()
