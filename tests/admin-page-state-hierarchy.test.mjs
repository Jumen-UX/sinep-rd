...
test('user access administration uses canonical header feedback actions and hierarchy', async () => {
  const page = await read('src/features/access/admin/UserAccessPage.tsx')

  assert.match(page, /<PageHeader/)
  assert.match(page, /\{ label: 'Configuración', href: '\/admin\/configuracion' \}/)
  assert.match(page, /<PageState[\s\S]*kind="loading"/)
  assert.match(page, /<PageState kind="error"/)
  assert.match(page, /kind="empty"/)
  assert.match(page, /<Alert tone="success"/)
  assert.match(page, /<StatusBadge/)
  assert.match(page, /<Button/)
  assert.match(page, /<h2>Accesos activos y estado de cuenta<\/h2>/)
  assert.match(page, /<h3>\{user\.full_name/)
  assert.match(page, /<h3>\{role\.role_name\}<\/h3>/)
  assert.doesNotMatch(page, /className="admin-top-header"/)
  assert.doesNotMatch(page, /className="admin-welcome-panel"/)
  assert.doesNotMatch(page, /className="empty-state"/)
  assert.doesNotMatch(page, /className="error-box"/)
  assert.doesNotMatch(page, /className="button button-/)
  assert.doesNotMatch(page, /className="role-pill"/)
})

test('review queue uses canonical header feedback actions filters and hierarchy', async () => {
  const page = await read('src/features/review/admin/ReviewQueuePage.tsx')

  assert.match(page, /<PageHeader/)
  assert.match(page, /breadcrumbs=\{\[\{ label: 'Administración', href: '\/admin' \}, \{ label: 'Revisión' \}\]\}/)
  assert.match(page, /<Alert tone="danger"/)
  assert.match(page, /<Alert tone="success"/)
  assert.match(page, /<PageState compact kind="loading"/)
  assert.match(page, /kind="empty"/)
  assert.match(page, /<StatusBadge/)
  assert.match(page, /<Button/)
  assert.match(page, /htmlFor="review-type-filter"/)
  assert.match(page, /htmlFor="review-status-filter"/)
  assert.match(page, /<h2 id="review-filters-heading"/)
  assert.match(page, /<h2 id="review-results-heading"/)
  assert.match(page, /<h3>\{item\.title \|\| item\.record_table\}<\/h3>/)
  assert.doesNotMatch(page, /className="admin-top-header"/)
  assert.doesNotMatch(page, /className="admin-welcome-panel"/)
  assert.doesNotMatch(page, /className="empty-state"/)
  assert.doesNotMatch(page, /className="error-box"/)
  assert.doesNotMatch(page, /className="success-box"/)
  assert.doesNotMatch(page, /className="role-pill"/)
  assert.doesNotMatch(page, /className="button button-/)
})
