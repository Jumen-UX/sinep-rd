...
test('missing clergy profile diagnostic uses a private definer behind an invoker facade', async () => {
  const hardening = await readRepoFile(
    'supabase/migrations/20260715194500_harden_missing_clergy_profile_diagnostic.sql',
  )

  assert.match(hardening, /internal\.admin_count_missing_clergy_profiles\(\)/)
  assert.match(hardening, /security definer/)
  assert.match(hardening, /auth\.uid\(\) is not null/)
  assert.match(hardening, /current_user_can_manage_person\('people\.view_private', oe\.person_id\)/)
  assert.match(hardening, /public\.admin_count_missing_clergy_profiles\(\)/)
  assert.match(hardening, /security invoker/)
  assert.match(hardening, /select internal\.admin_count_missing_clergy_profiles\(\)/)
  assert.match(hardening, /revoke all on function public\.admin_count_missing_clergy_profiles\(\) from anon/)
  assert.match(hardening, /grant execute on function public\.admin_count_missing_clergy_profiles\(\) to authenticated/)
})
