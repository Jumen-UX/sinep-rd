import assert from 'node:assert/strict'
import { readFile } from 'node:fs/promises'
import test from 'node:test'

const pagePath = 'src/app/(account)/cuenta/seguridad/page.tsx'
const managerPath = 'src/features/account/AccountSecurityManager.tsx'
const shellPath = 'src/features/account/AccountShell.tsx'
const cssPath = 'src/features/account/account-security.module.css'
const lowerCssPath = 'src/features/account/account-security-lower.module.css'

test('personal security route requires authentication and exposes real account controls', async () => {
  const [page, manager, shell] = await Promise.all([readFile(pagePath, 'utf8'), readFile(managerPath, 'utf8'), readFile(shellPath, 'utf8')])
  assert.match(page, /supabase\.auth\.getUser\(\)/)
  assert.match(page, /redirect\('\/admin\/login\?next=\/cuenta\/seguridad'\)/)
  assert.match(page, /email=\{user\.email \?\? 'Correo no disponible'\}/)
  assert.match(shell, /href: '\/cuenta\/seguridad'/)
  assert.match(manager, /supabase\.auth\.updateUser\(\{ password \}\)/)
  assert.match(manager, /supabase\.auth\.signOut\(\{ scope: 'others' \}\)/)
  assert.doesNotMatch(manager, /service_role|admin\.deleteUser|admin\.updateUserById/)
})

test('password change validates measurable criteria and prevents duplicate actions', async () => {
  const manager = await readFile(managerPath, 'utf8')
  assert.match(manager, /MIN_PASSWORD_LENGTH = 12/)
  assert.match(manager, /\[A-Z\]/)
  assert.match(manager, /\[a-z\]/)
  assert.match(manager, /\\d/)
  assert.match(manager, /\[\^A-Za-z0-9\]/)
  assert.match(manager, /password === confirmation/)
  assert.match(manager, /disabled=\{!passwordValid \|\| busy !== null\}/)
  assert.match(manager, /Completa todos los requisitos y confirma la contraseña/)
  assert.match(manager, /role="alert"/)
  assert.match(manager, /role="status"/)
})

test('security refinement exposes visible fields strength guidance and confirmation semantics', async () => {
  const [manager, css] = await Promise.all([readFile(managerPath, 'utf8'), readFile(cssPath, 'utf8')])
  assert.match(manager, /placeholder="Escribe una contraseña nueva"/)
  assert.match(manager, /placeholder="Repite la nueva contraseña"/)
  assert.match(manager, /function EyeIcon/)
  assert.match(manager, /Mostrar nueva contraseña/)
  assert.match(manager, /Ocultar nueva contraseña/)
  assert.match(manager, /aria-pressed=\{showPassword\}/)
  assert.match(manager, /className=\{styles\.strengthBadge\}/)
  assert.match(manager, /role="progressbar"/)
  assert.match(manager, /Fortaleza de la contraseña/)
  assert.match(manager, /Agrega un símbolo especial, por ejemplo:/)
  assert.match(manager, /Las contraseñas coinciden/)
  assert.match(manager, /Las contraseñas no coinciden/)
  assert.match(manager, /aria-describedby="password-strength password-requirements"/)
  assert.match(manager, /Consejos de seguridad/)
  assert.match(manager, /Esta sesión permanece activa/)
  assert.match(manager, /emailConfirmed \? 'Verificado' : 'Pendiente'/)
  assert.match(css, /:global\(html\[data-theme='light'\]\) \.securityStack/)
  assert.match(css, /\.inputGroup\{[^}]*position:relative/s)
  assert.match(css, /\.visibilityButton\{[^}]*position:absolute/s)
  assert.match(css, /\.strengthTrack\{[^}]*height:12px/s)
  assert.match(css, /max-width:680px/)
})

test('lower security controls confirm sensitive actions and communicate honest capability states', async () => {
  const [manager, lowerCss] = await Promise.all([readFile(managerPath, 'utf8'), readFile(lowerCssPath, 'utf8')])
  assert.match(manager, /confirmationDialogRef/)
  assert.match(manager, /dialog\.showModal\(\)/)
  assert.match(manager, /requestAnimationFrame\(\(\) => cancelDialogButtonRef\.current\?\.focus\(\)\)/)
  assert.match(manager, /Se cerrarán todas las demás sesiones de tu cuenta/)
  assert.match(manager, /Esta sesión permanecerá activa/)
  assert.match(manager, /onClick=\{\(\) => setConfirmSessions\(true\)\}/)
  assert.match(manager, /Próximamente/)
  assert.match(manager, /Aún no disponible/)
  assert.match(manager, /Control de sesiones/)
  assert.match(manager, /Cierre global disponible/)
  assert.match(manager, /El detalle individual de dispositivos todavía no está disponible/)
  assert.doesNotMatch(manager, /MFA pendiente|Gestión disponible|Dispositivos y sesiones/)
  assert.match(lowerCss, /\.passwordAction:disabled/)
  assert.match(lowerCss, /opacity:1!important/)
  assert.match(lowerCss, /\.confirmDialog::backdrop/)
  assert.match(lowerCss, /min-height:176px/)
  assert.match(lowerCss, /@media\(max-width:800px\)/)
})

test('security confirmation dialog is centered safe animated and reports progress', async () => {
  const [manager, lowerCss] = await Promise.all([readFile(managerPath, 'utf8'), readFile(lowerCssPath, 'utf8')])
  assert.match(manager, /function LockIcon/)
  assert.match(manager, /busy === 'sessions' \? <><span className=\{lowerStyles\.spinner\}/)
  assert.match(manager, /disabled=\{busy === 'sessions'\}/)
  assert.match(manager, /if \(busy === 'sessions'\) event\.preventDefault\(\)/)
  assert.match(manager, /Todas las demás sesiones fueron cerradas correctamente/)
  assert.match(manager, /window\.setTimeout\(\(\) => setMessage\(null\), 4000\)/)
  assert.match(manager, /lowerStyles\.toast/)
  assert.match(lowerCss, /\.confirmDialog\{[^}]*position:fixed[^}]*inset:50% auto auto 50%/s)
  assert.match(lowerCss, /transform:translate\(-50%,-50%\) scale\(\.97\)/)
  assert.match(lowerCss, /background:rgba\(5,8,15,\.78\)/)
  assert.match(lowerCss, /backdrop-filter:blur\(6px\) brightness\(\.72\)/)
  assert.match(lowerCss, /padding:30px/)
  assert.match(lowerCss, /\.dialogIcon\{[^}]*width:48px[^}]*height:48px/s)
  assert.match(lowerCss, /@keyframes dialog-in/)
  assert.match(lowerCss, /@keyframes spinner-rotate/)
  assert.match(lowerCss, /\.toast\{[^}]*position:fixed/s)
})

test('security toast remains above accessibility and mobile navigation without blocking the page', async () => {
  const lowerCss = await readFile(lowerCssPath, 'utf8')
  assert.match(lowerCss, /\.toast\{[^}]*bottom:calc\(5\.75rem \+ env\(safe-area-inset-bottom\)\)/s)
  assert.match(lowerCss, /\.toast\{[^}]*z-index:10010/s)
  assert.match(lowerCss, /\.toast\{[^}]*pointer-events:none/s)
  assert.match(lowerCss, /width:min\(420px,calc\(100vw - 2rem\)\)/)

  const mobile800Start = lowerCss.indexOf('@media(max-width:800px)')
  const mobile480Start = lowerCss.indexOf('@media(max-width:480px)')
  const reducedMotionStart = lowerCss.indexOf('@media(prefers-reduced-motion:reduce)')

  assert.notEqual(mobile800Start, -1)
  assert.notEqual(mobile480Start, -1)
  assert.notEqual(reducedMotionStart, -1)
  assert.ok(mobile800Start < mobile480Start)
  assert.ok(mobile480Start < reducedMotionStart)

  const mobile800Css = lowerCss.slice(mobile800Start, mobile480Start)
  const mobile480Css = lowerCss.slice(mobile480Start, reducedMotionStart)

  assert.match(mobile800Css, /\.toast\{[^}]*bottom:calc\(7\.25rem \+ env\(safe-area-inset-bottom\)\)/s)
  assert.match(mobile480Css, /\.toast\{[^}]*bottom:calc\(7\.5rem \+ env\(safe-area-inset-bottom\)\)/s)
})

test('security workspace remains responsive touch-safe reduced-motion-aware and does not invent device inventory', async () => {
  const [manager, css, lowerCss] = await Promise.all([readFile(managerPath, 'utf8'), readFile(cssPath, 'utf8'), readFile(lowerCssPath, 'utf8')])
  assert.match(manager, /detalle individual de dispositivos todavía no está disponible/)
  assert.match(manager, /Autenticación en dos pasos/)
  assert.match(css, /min-height:44px/)
  assert.match(css, /@media\(max-width:800px\)/)
  assert.match(css, /@media\(max-width:480px\)/)
  assert.match(css, /@media\(prefers-reduced-motion:reduce\)/)
  assert.match(lowerCss, /@media\(prefers-reduced-motion:reduce\)/)
  assert.match(lowerCss, /grid-template-columns:1fr/)
  assert.match(lowerCss, /\.confirmDialog\{width:calc\(100% - 24px\)/)
})
