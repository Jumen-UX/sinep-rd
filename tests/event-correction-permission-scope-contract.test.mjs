...
test('event corrections require permission, scope and row serialization', () => {
  ...
  assert.match(revisionMigration, /constraint canonical_event_revisions_unique_number unique\(event_id, revision_number\)/i)
...
test('revision history is administrative and scope restricted', () => {
  ...
  assert.match(historyMigration, /order by revision\.revision_number desc/i)
})
