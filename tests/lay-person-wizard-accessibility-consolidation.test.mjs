...
test('lay person wizard owns five explicit stages without the automatic DOM observer wrapper', () => {
  ...
  assert.match(page, /compactOnMobile/)
...
test('lay person wizard owns loading error success and busy semantics', () => {
  ...
  assert.match(page, /<FormErrorSummary errors=\{formErrors\}/)
  assert.match(page, /fieldId: 'lay-existing-person'/)
  assert.match(page, /fieldId: 'lay-first-name'/)
  assert.match(page, /fieldId: 'lay-last-name'/)
...
test('lay person review and navigation remain semantic and submit natively', () => {
  ...
  assert.match(page, /<Button[\s\S]*loading=\{saving\}[\s\S]*loadingLabel="Guardando…"[\s\S]*type="submit"/)
...
test('lay person identity and persistence contracts remain unchanged', () => {
  ...
  assert.match(page, /onModeChange=\{\(nextMode\) => \{ setMode\(nextMode\); setFormErrors\(\[\]\) \}\}/)
  assert.match(page, /selectId="lay-existing-person"/)
