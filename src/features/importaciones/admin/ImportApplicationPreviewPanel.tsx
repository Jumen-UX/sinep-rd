...
export default function ImportApplicationPreviewPanel({ rows, serverCanApply }: Props) {
  ...
  const ready = preview.canApply && serverCanApply
    ...
    <div className="compact-section" aria-label="Vista previa de aplicación">
      <div className="section-heading">
        <div>
          ...
          <h3>{ready ? 'Operaciones resueltas' : 'La aplicación todavía está bloqueada'}</h3>
          <p className="meta">
            Esta vista no modifica datos. Resume las operaciones determinadas durante la validación vigente.
          </p>
        </div>
        <span className="role-pill">{ready ? 'Lista para aplicar' : 'Requiere atención'}</span>
      </div>
      ...
      {preview.canApply && !serverCanApply && (
        <div className="admin-info-box">
          <span>
