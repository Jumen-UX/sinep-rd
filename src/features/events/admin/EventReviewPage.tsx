    ...
    <main className="container dashboard-page event-review-page">
      ...
      <section className="dashboard-hero card review-hero">
        <div>
          ...
          <h1>{event.title}</h1>
          <p className="lead">Revisa datos, alcance, fuente y plan de impacto antes de aprobar. La aplicación es un paso posterior y explícito.</p>
        </div>
        <div className="review-summary">
          ...
          <span className="meta">{verificationLabel(event.verification_status)}</span>
        </div>
      </section>
      ...
      <section className="review-layout">
        <div className="card dashboard-section">
          ...
          <div className="review-grid">
            ...
            <div className="review-card"><strong>Evidencia</strong><span className="meta">{evidenceLabel(event.evidence_status)}</span></div>
            <div className="review-card"><strong>Verificación</strong><span className="meta">{verificationLabel(event.verification_status)}</span></div>
            <div className="review-card"><strong>Fuente</strong><span className="meta">{event.source_name ?? '—'}</span></div>
            <div className="review-card"><strong>Fecha de revisión</strong><span className="meta">{formatDate(event.source_checked_at)}</span></div>
          ...
          <div className="section-heading"><div><p className="eyebrow">Alcance y destinos</p><h2>{isOrganizational ? 'Unidades organizativas' : 'Entidades relacionadas'}</h2></div></div>
        ...
        <aside className="card dashboard-section">
          ...
          <div className="check-grid">
            ...
            <CheckCard ok={checks.has_effective_date} title="Fecha efectiva" detail="Define desde cuándo produce efectos." />
            <CheckCard ok={checks.has_participant} title="Alcance" detail={isOrganizational ? 'Tiene unidad vinculada o define una creación válida.' : 'Tiene entidad principal vinculada.'} />
            <CheckCard ok={checks.has_source_reference} title="Fuente" detail="Tiene fuente o referencia declarada." />
            <CheckCard ok={checks.has_verification_contract} title="Verificación" detail="Una verificación confirmada conserva fuente y fecha de revisión." />
