...
export default function OfficeConfigurationPage() {
  ...
  async function loadData() {
    ...
    const [officeRes, requestRes] = await Promise.all([
      supabase.from('office_configurations').select('id,key,display_name,allowed_person_types,required_ordination_degree,allowed_clerical_statuses,allowed_episcopal_role_types,holder_cardinality,max_current_holders,office_scopes(name),office_categories(name)').eq('status','active').order('display_name'),
      supabase.from('change_requests').select('id,title,description,proposed_data,status,created_at,submitted_at').eq('target_table','office_configurations').in('status',['pending_review','needs_changes']).order('submitted_at',{ ascending:false, nullsFirst:false }),
    ])
  ...
  return <main className="container dashboard-page admin-config-page">
    ...
    <section className="dashboard-hero card"><div><p className="eyebrow">Configuración canónica</p><h1>Cargos y oficios</h1><p className="lead">Define quién puede ocupar cada cargo y cuántos titulares vigentes admite.</p></div></section>
    ...
    <section className="card dashboard-section"><div className="section-heading"><div><p className="eyebrow">Nuevo cargo</p><h2>Identidad y ámbito</h2></div></div>
      <form className="admin-form admin-config-form" onSubmit={submit}>
        <input name="display_name" placeholder="Nombre visible, ej. Párroco" required />
        <input name="key" placeholder="Clave opcional" />
        <input name="base_role_name" placeholder="Cargo base" />
        <select name="category_key" defaultValue="pastoral">{categories.map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select>
        <select name="scope_key" defaultValue="diocesan">{scopes.map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select>
        <textarea name="description" placeholder="Descripción del cargo" />

        <section className="card compact-section"><p className="eyebrow">Elegibilidad sacramental</p><h3>Grado mínimo del Orden</h3>
          <select value={degree} onChange={(event) => setDegree(event.target.value)}><option value="none">Ninguno</option><option value="diaconate">Diaconado</option><option value="presbyterate">Presbiterado</option><option value="episcopate">Episcopado</option></select>
          <div className="role-list">{personTypes.map(([v,l]) => <label className="role-pill" key={v}><input name={`person_type_${v}`} type="checkbox" defaultChecked={degree === 'none' || v === 'priest'} /> {l}</label>)}</div>
          <label className="role-pill"><input name="requires_clergy" type="checkbox" defaultChecked={degree !== 'none'} /> Requiere clero</label>
        </section>
        ...
        <section className="card compact-section"><p className="eyebrow">Ministerio episcopal</p><h3>Funciones requeridas o admitidas</h3><p className="meta">Déjalo vacío cuando el cargo no dependa de una función episcopal concreta.</p><div className="role-list">{episcopalRoles.map(([v,l]) => <label className="role-pill" key={v}><input name={`episcopal_role_${v}`} type="checkbox" /> {l}</label>)}</div></section>
        <section className="card compact-section"><p className="eyebrow">Cardinalidad</p><h3>Titulares vigentes</h3><select value={cardinality} onChange={(event) => setCardinality(event.target.value)}><option value="single">Titular único</option><option value="multiple">Varios titulares</option></select>{cardinality === 'multiple' && <input name="max_current_holders" type="number" min="2" placeholder="Máximo opcional" />}<p className="meta">El titular único cerrará automáticamente al anterior cuando entre uno nuevo.</p></section>

        <textarea name="reason" placeholder="Justificación o motivo administrativo" /><input name="source_name" placeholder="Fuente o documento" /><input name="source_url" placeholder="URL de fuente" />
        <label className="role-pill"><input name="is_elective" type="checkbox" /> Es electivo</label>
        <div className="actions"><button className="button button-primary" name="intent" value="admin_create" disabled={saving}>Crear cargo oficial</button><button className="button button-secondary" name="intent" value="suggest" disabled={saving}>Sugerir para revisión</button></div>
      </form>
    </section>
