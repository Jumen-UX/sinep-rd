...
function flattenTree(units: OrganizationUnit[]): TreeRow[] {
  ...
  for (const siblings of byParent.values()) {
    siblings.sort((a, b) => a.sort_order - b.sort_order || a.name.localeCompare(b.name, 'es'))
  }
...
function lifecycleOptions(unit: OrganizationUnit): LifecycleOption[] {
  if (unit.status === 'draft') {
    return [
      { action: 'approve', label: 'Aprobar unidad' },
  ...
  if (unit.status === 'active') {
    return [
      unit.visibility === 'public'
        ? { action: 'unpublish', label: 'Retirar publicación' }
  ...
  if (unit.status === 'inactive') {
    return [
      { action: 'restore_draft', label: 'Restaurar como borrador' },
      { action: 'archive', label: 'Archivar', destructive: true },
    ]
  }
...
export default function OrganizationUnitManagerPage() {
  ...
  const filteredUnits = useMemo(
    () => catalogs.units.filter((unit) => (
      unit.ecclesiastical_entity_id === selectedEntityId
      && unit.organization_chart_id === selectedChartId
    )),
    [catalogs.units, selectedChartId, selectedEntityId],
  )
  ...
  const excludedParentIds = useMemo(
    () => descendantIds(filteredUnits, selectedUnitId),
  ...
  async function refresh(preferredUnitId?: string | null) {
    ...
    const entityId = selectedEntityId
      || nextCatalogs.entities.find((entity) => entity.slug === 'arquidiocesis-metropolitana-de-santo-domingo')?.id
      || nextCatalogs.entities[0]?.id
      || ''
    const chartId = selectedChartId
      || nextCatalogs.charts.find((chart) => chart.key === 'diocesan_pastoral')?.id
    ...
    const unitId = preferredUnitId ?? selectedUnitId
    const selected = nextCatalogs.units.find((unit) => unit.id === unitId)
    if (selected) {
      setSelectedUnitId(selected.id)
      setForm(formFromUnit(selected))
    } else {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (cancelled) return
        setCatalogs(nextCatalogs)
        setSelectedEntityId(nextCatalogs.entities.find((entity) => entity.slug === 'arquidiocesis-metropolitana-de-santo-domingo')?.id ?? nextCatalogs.entities[0]?.id ?? '')
        setSelectedChartId(nextCatalogs.charts.find((chart) => chart.key === 'diocesan_pastoral')?.id ?? nextCatalogs.charts[0]?.id ?? '')
      } catch (loadError) {
  ...
  function changeChart(chartId: string) {
    setSelectedChartId(chartId)
    setSelectedUnitId(null)
    setForm(emptyForm())
  }
  ...
  return (
    <main className="admin-organization-page" id="top">
      <header className="admin-top-header">
        ...
        <div className="admin-top-actions">
          ...
          <Link className="button button-secondary" href="/admin/asignaciones">Nombramientos</Link>
        </div>
      </header>

      <section className="admin-welcome-panel">
        <div>
          <p className="eyebrow">Organigramas</p>
          <h1>Unidades, jerarquías y ámbitos</h1>
          <p className="lead">Gestiona pastorales, curia, gobierno y otras unidades funcionales sin mezclarlas con la estructura territorial de la diócesis.</p>
          <div className="role-list admin-role-list">
            <span className="role-pill">Edición separada</span>
            <span className="role-pill">Aprobación explícita</span>
            <span className="role-pill">Publicación auditada</span>
          </div>
        </div>
        <div className="admin-welcome-illustration" aria-hidden="true">▦</div>
      </section>

      <section className="admin-stat-strip" aria-label="Resumen organizativo">
        <a href="#organization-tree"><span>▥</span><strong>{filteredUnits.length}</strong><small>Unidades</small></a>
        <a href="#organization-tree"><span>⌂</span><strong>{rootCount}</strong><small>Raíces</small></a>
        <a href="#organization-tree"><span>✓</span><strong>{activeCount}</strong><small>Activas</small></a>
        <a href="#organization-tree"><span>◌</span><strong>{draftCount}</strong><small>Borradores</small></a>
      </section>
      ...
      <section className="card dashboard-section">
        <div className="section-heading">
          <div><p className="eyebrow">Ámbito de trabajo</p><h2>Diócesis y organigrama</h2></div>
          <span className="meta">{selectedEntity?.name ?? 'Sin diócesis'} · {selectedChart?.name ?? 'Sin organigrama'}</span>
        </div>
        <div className="admin-form admin-config-form">
          <label>Diócesis
            <select value={selectedEntityId} onChange={(event) => changeEntity(event.target.value)}>
          ...
          <label>Organigrama
            <select value={selectedChartId} onChange={(event) => changeChart(event.target.value)}>
          ...
          <p className="meta">{selectedChart?.description ?? 'Cada organigrama mantiene su propia jerarquía y sus cargos compatibles.'}</p>
        </div>
      </section>

      <section className="card dashboard-section" id="organization-tree">
        <div className="section-heading">
          <div><p className="eyebrow">Jerarquía</p><h2>Árbol de unidades</h2></div>
          <button className="button button-primary" onClick={() => beginNewUnit()} type="button">+ Nueva unidad</button>
        </div>
        {treeRows.length === 0 ? <div className="empty-state"><strong>Este organigrama todavía no tiene unidades.</strong><span>Crea la primera unidad raíz.</span></div> : (
          <div className="table-wrap">
            <table className="data-table dashboard-list-table">
              <thead><tr><th>Unidad</th><th>Área pastoral</th><th>Estado</th><th>Visibilidad</th><th>Orden</th><th>Acciones</th></tr></thead>
              <tbody>{treeRows.map(({ unit, depth }) => {
                const area = catalogs.pastoralAreas.find((item) => item.id === unit.pastoral_area_id)
                return <tr key={unit.id}>
                  <td style={{ paddingLeft: `${0.75 + depth * 1.5}rem` }}><strong>{depth > 0 ? '↳ ' : ''}{unit.name}</strong><small className="meta">{unit.key}</small></td>
                  <td>{area?.name ?? 'Unidad cabecera'}</td>
                  <td>{statusLabel(unit.status)}{unit.is_current ? '' : ' · histórica'}</td>
                  <td>{visibilityLabel(unit.visibility)}</td>
                  <td>{unit.sort_order}</td>
                  <td><button className="button button-secondary" onClick={() => selectUnit(unit)} type="button">Editar y revisar</button>{' '}<button className="button button-secondary" onClick={() => beginNewUnit(unit.id)} type="button">Agregar debajo</button></td>
                </tr>
              })}</tbody>
            </table>
          </div>
      ...
      <section className="card dashboard-section" id="organization-unit-form">
        <div className="section-heading">
          <div><p className="eyebrow">{selectedUnitId ? 'Editar contenido' : 'Nueva unidad'}</p><h2>{selectedUnitId ? form.name : 'Datos organizativos'}</h2></div>
          {selectedUnitId && <button className="button button-secondary" onClick={() => beginNewUnit()} type="button">Cancelar edición</button>}
        </div>

        {selectedUnit && <div className="admin-form admin-config-form" aria-label="Ciclo de vida de la unidad">
          <div><strong>Estado actual</strong><p className="meta">{statusLabel(selectedUnit.status)} · {visibilityLabel(selectedUnit.visibility)} · {selectedUnit.is_current ? 'Vigente' : 'Histórica'}</p></div>
          <div className="admin-top-actions">
            {lifecycleOptions(selectedUnit).map((option) => <button
              className={option.destructive ? 'button button-secondary' : 'button button-primary'}
              disabled={transitioning !== null || saving}
              key={option.action}
              onClick={() => void handleLifecycle(option.action, option.destructive)}
              type="button"
            >{transitioning === option.action ? 'Procesando…' : option.label}</button>)}
          </div>
          <p className="meta">Aprobar y publicar son operaciones separadas, con permisos y auditoría independientes.</p>
        </div>}
        ...
        <form className="admin-form admin-config-form" onSubmit={handleSubmit}>
          ...
          <label>Clave técnica opcional<input value={form.key} onChange={(event) => setForm((current) => ({ ...current, key: event.target.value }))} placeholder="Se genera automáticamente" /></label>
          <label>Unidad superior<select value={form.parentUnitId} onChange={(event) => setForm((current) => ({ ...current, parentUnitId: event.target.value }))}><option value="">Unidad raíz</option>{flattenTree(parentOptions).map(({ unit, depth }) => <option key={unit.id} value={unit.id}>{'— '.repeat(depth)}{unit.name}</option>)}</select></label>
          <label>Área pastoral<select value={form.pastoralAreaId} onChange={(event) => setForm((current) => ({ ...current, pastoralAreaId: event.target.value }))}><option value="">Sin área pastoral / unidad cabecera</option>{catalogs.pastoralAreas.map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</select></label>
          <label>Orden<input min="0" type="number" value={form.sortOrder} onChange={(event) => setForm((current) => ({ ...current, sortOrder: event.target.value }))} /></label>
          <label>Vigente desde<input type="date" value={form.validFrom} onChange={(event) => setForm((current) => ({ ...current, validFrom: event.target.value }))} /></label>
          <label>Vigente hasta<input type="date" value={form.validTo} onChange={(event) => setForm((current) => ({ ...current, validTo: event.target.value }))} /></label>
          <label className="role-pill"><input checked={form.isCurrent} onChange={(event) => setForm((current) => ({ ...current, isCurrent: event.target.checked }))} type="checkbox" /> Unidad vigente</label>
          ...
          <p className="meta">El guardado modifica contenido y jerarquía. No aprueba ni publica la unidad.</p>
          <button className="button button-primary" disabled={saving || transitioning !== null || !selectedEntityId || !selectedChartId}>{saving ? 'Guardando…' : selectedUnitId ? 'Guardar contenido' : 'Crear borrador'}</button>
        </form>
      </section>
    </main>
  )
