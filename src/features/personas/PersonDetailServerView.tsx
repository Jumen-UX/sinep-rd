    ...
    <main className="container detail-page">
      ...
      <section className="card detail-section"><h2>Cargos configurados</h2>{positions.length === 0 ? <p className="meta">Todavía no hay cargos configurados para esta persona.</p> : <div className="table-wrap"><table className="data-table dashboard-list-table"><thead><tr><th>Cargo</th><th>Organigrama</th><th>Entidad / unidad</th><th>Período</th><th>Estado</th><th>Predecesor</th><th>Sucesor</th></tr></thead><tbody>
        {positions.map((position) => <tr key={position.id}>
          ...
          <td><EntityLink name={position.ecclesiastical_entity_name ?? position.organization_unit_name ?? position.organization_unit_name} slug={position.ecclesiastical_entity_slug ?? position.organization_unit_slug} /></td>
      ...
      <section className="detail-grid">
        <article className="card detail-section"><h2>Nombramientos actuales</h2>{appointments.length === 0 ? <p className="meta">No hay nombramientos públicos activos.</p> : <div className="timeline-list">{appointments.map((appointment) => <div className="timeline-item" key={appointment.id}><strong>{appointment.office_name ?? 'Cargo'}</strong><EntityLink name={appointment.entity_name ?? appointment.organization_unit_name} slug={appointment.entity_slug ?? appointment.organization_unit_slug} /><small>Desde {formatDate(appointment.start_date)} · {metricYears(appointment.start_date, person.death_date)} años en el cargo</small>{appointment.notes_public && <span className="meta">{appointment.notes_public}</span>}</div>)}</div>}</article>
      ...
      <section className="card detail-section"><h2>Movimientos pastorales e institucionales</h2>{movements.length === 0 ? <p className="meta">Todavía no hay movimientos históricos públicos.</p> : <div className="timeline-list">{movements.map((movement) => <div className="timeline-item" key={movement.id}><strong>{movement.title ?? movement.movement_type ?? 'Movimiento'}</strong><EntityLink name={movement.entity_name ?? movement.organization_unit_name} slug={movement.entity_slug ?? movement.organization_unit_slug} /><small>{formatDate(movement.effective_date)} — {movement.end_date ? formatDate(movement.end_date) : 'actual'}</small>{movement.description && <span className="meta">{movement.description}</span>}</div>)}</div>}</section>
    </main>
  )
}
