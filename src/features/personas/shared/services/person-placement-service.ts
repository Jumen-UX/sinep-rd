...
export type PersonPlacementCatalogs = {
...
export async function loadPersonPlacementCatalogs(supabase: SupabaseClient): Promise<PersonPlacementCatalogs> {
...
export async function uploadPersonPhoto(
  ...
  const safeFolder = folder.replace(/[^a-z0-9-]/gi, '').toLowerCase() || 'personas'
...
export async function removePersonPhoto(supabase: SupabaseClient, photoPath: string | null | undefined) {
  if (!photoPath) return
  const { error } = await supabase.storage.from(PHOTO_BUCKET).remove([photoPath])
  if (error) console.error('No se pudo limpiar la fotografía huérfana de la persona.', error)
}
