...
function PriestSaveSuccessNotice({
  message,
  internalCode,
  personId,
  slug,
}: PriestSaveSuccessNoticeProps) {
  ...
  return (
    <section
      aria-atomic="true"
      aria-live="polite"
      className="success-box admin-wizard-success"
      role="status"
    >
      <div>
        <strong>{message}</strong>
        {internalCode ? <span>Código interno: {internalCode}</span> : null}
      </div>
      <div className="admin-actions" aria-label="Acciones posteriores al registro">
        {adminHref ? (
          <Link className="button button-primary" href={adminHref}>
            Abrir ficha administrativa
          </Link>
        ) : null}
        {publicHref ? (
          <Link className="button button-secondary" href={publicHref}>
            Ver ficha pública
          </Link>
        ) : null}
      </div>
...
export default function PriestWizardPage() {
  ...
  const [levelFilterMessage, setLevelFilterMessage] = useState(
    'Selecciona una entidad para filtrar los cargos permitidos.',
  )
  ...
  const filteredOfficeConfigs = quickEntityId
    ...
    : []
  const notIdentifiedFields = Array.isArray(draftValues.not_identified_fields)
    ? draftValues.not_identified_fields
    : []
  ...
  function handleDraftChange(event: FormEvent<HTMLFormElement>) {
    ...
    setDraftValues((current) => {
      ...
      if (
        ...
        && target.name === 'not_identified_fields'
      ) {
        const selected = Array.isArray(current.not_identified_fields)
          ? current.not_identified_fields
          : []
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (catalogs.deacons.length === 0 && !window.localStorage.getItem(DRAFT_KEY)) {
          setMode('new-priest')
        }
      } catch (loadError) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        setSelectedDeaconId(
          typeof parsed.existing_deacon_person_id === 'string'
            ? parsed.existing_deacon_person_id
            : '',
        )
        setIncardinationId(
          typeof parsed.incardination_entity_id === 'string'
            ? parsed.incardination_entity_id
            : '',
        )
        setServiceId(
          typeof parsed.current_service_entity_id === 'string'
            ? parsed.current_service_entity_id
            : '',
        )
        setQuickEntityId(
          typeof parsed.quick_entity_id === 'string'
            ? parsed.quick_entity_id
            : typeof parsed.current_service_entity_id === 'string'
              ? parsed.current_service_entity_id
              : '',
        )
        setQuickOfficeConfigId(
          typeof parsed.quick_office_configuration_id === 'string'
            ? parsed.quick_office_configuration_id
            : '',
        )
      }
    } catch {
      ...
      try {
        ...
        setLevelFilterMessage(
          officeError instanceof Error
            ? officeError.message
            : 'No se pudieron filtrar los cargos.',
        )
      }
    }
  ...
  const completionItems = [
    {
      ...
      complete: mode === 'existing-deacon'
        ? Boolean(selectedDeaconId)
        : Boolean(fieldValue('first_name') && fieldValue('last_name')),
    },
    { label: 'Tipo de sacerdote', complete: Boolean(priestType) },
    {
      label: 'Ordenación sacerdotal',
      complete: Boolean(fieldValue('priestly_ordination_date'))
        || notIdentifiedFields.includes('priestly_ordination_date'),
    },
    {
      label: 'Pertenencia',
      complete: priestType === 'religious'
        ? Boolean(fieldValue('religious_institute_name'))
        : Boolean(incardinationId),
    },
    {
      label: 'Servicio actual',
      complete: Boolean(serviceId)
        || notIdentifiedFields.includes('current_service_entity_id'),
    },
  ]
  ...
  function validateCurrentStep() {
    ...
    if (
      ...
      && !fieldValue('religious_institute_name')
    ) {
    ...
    if (
      step === 2
      && !fieldValue('priestly_ordination_date')
      && !notIdentifiedFields.includes('priestly_ordination_date')
    ) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const displayName = selectedDeacon?.display_name || buildDisplayName([
      form.get('first_name'),
    ...
    try {
      const photoFile = form.get('photo_file') instanceof File
        ? form.get('photo_file') as File
        : null
      const uploadedPhoto = photoFile
        ? await uploadPriestPhoto(supabase, photoFile, slug)
      ...
      const payload = {
        ...
        second_last_name: emptyToNull(form.get('second_last_name'))
          ?? selectedDeacon?.second_last_name
          ?? null,
        ...
        biography_public: emptyToNull(form.get('biography_public'))
          ?? selectedDeacon?.biography_public
          ?? null,
      ...
      setMessage(
        existingDeaconId
          ? 'La ficha del diácono fue completada como sacerdote.'
          : 'Sacerdote creado correctamente.',
      )
    } catch (submitError) {
  ...
  return (
    <main
      aria-busy={saving}
      aria-labelledby="priest-wizard-title"
      className="container dashboard-page admin-config-page"
      id="top"
    >
      <nav className="admin-entity-breadcrumbs" aria-label="Ruta del asistente">
        <Link href="/admin/personas">Personas</Link>
        <span aria-hidden="true">›</span>
        <Link href="/admin/nuevo">Agregar ficha</Link>
        <span aria-hidden="true">›</span>
        <strong aria-current="page">Sacerdote</strong>
      </nav>

      <section className="card admin-wizard-header">
        <div>
          <p className="eyebrow">Asistente de creación</p>
          <h1 id="priest-wizard-title">Registrar sacerdote</h1>
          <p className="lead">
            Busca primero una persona existente. Las actualizaciones posteriores se realizan
            desde la ficha administrativa.
          </p>
        </div>
        <div className="role-list admin-role-list" aria-label="Características del asistente">
          <span className="role-pill">Borrador automático</span>
          <span className="role-pill">Catálogos guiados</span>
          <span className="role-pill">5 etapas</span>
        </div>
      </section>

      {error ? (
        <div className="error-box" id="priest-wizard-error" role="alert" aria-live="assertive">
          {error}
        </div>
      ) : null}
      ...
      <div className="admin-wizard-layout">
        ...
        <form
          aria-busy={saving}
          aria-describedby={error ? 'priest-wizard-error' : undefined}
        ...
        >
          ...
          <section hidden={step !== 1}>
            ...
            {mode === 'new-priest' ? (
              <div className="admin-form-fields-grid">
                <label>
                  Primer nombre
                  <input autoComplete="given-name" name="first_name" defaultValue={fieldValue('first_name')} />
                </label>
                <label>
                  Segundo nombre
                  <input autoComplete="additional-name" name="middle_name" defaultValue={fieldValue('middle_name')} />
                </label>
                <label>
                  Primer apellido
                  <input autoComplete="family-name" name="last_name" defaultValue={fieldValue('last_name')} />
                </label>
                <label>
                  Segundo apellido
                  <input name="second_last_name" defaultValue={fieldValue('second_last_name')} />
                </label>
              </div>
            ) : null}
            ...
            <div className="empty-state" role="status" aria-atomic="true" aria-live="polite">
              ...
              <span>{namePreview || 'Se generará automáticamente.'}</span>
            </div>
            ...
            <div className="admin-form-fields-grid">
              ...
              <label>
                ...
                <input
                  ...
                  name="birth_date"
                  type="date"
                  defaultValue={fieldValue('birth_date', selectedDeacon?.birth_date ?? '')}
                />
              </label>
              <label>
                Lugar de nacimiento
                <input
                  name="birth_place"
                  defaultValue={fieldValue('birth_place', selectedDeacon?.birth_place ?? '')}
                />
              </label>
              <label>
                Correo
                <input autoComplete="email" name="contact_email" type="email" defaultValue={fieldValue('contact_email')} />
              </label>
              <label>
                Teléfono principal
                <input autoComplete="tel" name="primary_phone" defaultValue={fieldValue('primary_phone')} />
              </label>
              <label>
                Teléfono alterno
                <input name="secondary_phone" defaultValue={fieldValue('secondary_phone')} />
              </label>
              <label>
                Tipo de documento
              ...
              <label>
                Número de documento
                <input name="validation_value" defaultValue={fieldValue('validation_value')} />
              </label>
              <label>
                País del documento
                <select
                  name="validation_country"
                  defaultValue={fieldValue('validation_country', 'República Dominicana')}
                >
            ...
            <label>
              ...
              <textarea
                ...
                defaultValue={fieldValue('biography_public', selectedDeacon?.biography_public ?? '')}
              />
            </label>

            <details className="card compact-section">
              <summary>Datos familiares y notas</summary>
              <div className="admin-form-fields-grid">
                <label>
                  Nombre del padre
                  <input name="father_name" defaultValue={fieldValue('father_name')} />
                </label>
                <label>
                  Nombre de la madre
                  <input name="mother_name" defaultValue={fieldValue('mother_name')} />
                </label>
              </div>
              <label>
                Notas familiares
                <textarea name="family_notes" defaultValue={fieldValue('family_notes')} />
              </label>
              <label>
                Apuntes biográficos
                <textarea name="biography_notes" defaultValue={fieldValue('biography_notes')} />
              </label>
            </details>
          </section>
          ...
          <section hidden={step !== 2}>
            ...
            <fieldset className="clergy-option-fieldset">
              <legend>Tipo de sacerdote</legend>
              <div className="admin-choice-grid">
                <label className="role-pill">
                  <input
                    checked={priestType === 'diocesan'}
                    name="priest_type"
                    onChange={() => {
                      setPriestType('diocesan')
                      setDraftField('priest_type', 'diocesan')
                    }}
                    type="radio"
                    value="diocesan"
                  />
                  <span>
                    <strong>Sacerdote diocesano</strong>
                    <small>Incardinado en una jurisdicción.</small>
                  </span>
                </label>
                <label className="role-pill">
                  <input
                    checked={priestType === 'religious'}
                    name="priest_type"
                    onChange={() => {
                      setPriestType('religious')
                      setDraftField('priest_type', 'religious')
                    }}
                    type="radio"
                    value="religious"
                  />
                  <span>
                    ...
                    <small>Pertenece a una congregación u orden.</small>
                  </span>
                </label>
              </div>
            </fieldset>
            ...
            {priestType === 'religious' ? (
              <div className="card compact-section">
                <label>
                  Congregación, instituto u orden
                  <input
                    name="religious_institute_name"
                    defaultValue={fieldValue('religious_institute_name')}
                  />
                </label>
                <label>
                  Provincia religiosa
                  <input
                    name="religious_province_name"
                    defaultValue={fieldValue('religious_province_name')}
                  />
                </label>
              </div>
            ) : null}
            ...
            <div className="admin-form-fields-grid">
              <label>
                Ordenación diaconal
                <input
                  name="diaconal_ordination_date"
                  type="date"
                  defaultValue={fieldValue('diaconal_ordination_date')}
                />
              </label>
              <label>
                Ordenación sacerdotal
                <input
                  name="priestly_ordination_date"
                  type="date"
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) => {
                  setIncardinationId(event.target.value)
                  setDraftField('incardination_entity_id', event.target.value)
                }}
              >
                ...
                {entities
                  .filter((entity) => (
                    ['archdiocese', 'diocese', 'military_ordinariate']
                      .includes(entity.direct_entity_type_key ?? '')
                  ))
                  .map((entity) => (
            ...
            <p className="meta" role="status" aria-live="polite">
              {incardination?.hierarchy_path
                ...
                ?? 'Selecciona la jurisdicción correspondiente.'}
            </p>

            <label>
              Notas clericales
              <textarea name="clergy_notes" defaultValue={fieldValue('clergy_notes')} />
            </label>

            <fieldset className="clergy-option-fieldset">
              <legend>Datos no identificados</legend>
              <div className="clergy-option-list clergy-option-list--choices">
                {optionalFields
                  .filter((field) => field.key !== 'current_service_entity_id')
                  .map((field) => (
                    <label className="role-pill" key={field.key}>
                      <input
                        name="not_identified_fields"
                        type="checkbox"
                        value={field.key}
                        defaultChecked={notIdentifiedFields.includes(field.key)}
                      />
          ...
          <section hidden={step !== 3}>
            ...
            <p className="meta">
              La entidad se selecciona desde la estructura configurada; el cargo es opcional.
            </p>

            <StructureEntityPicker
              allowCreate
              createEntityTypeKey="parish"
              help="Selecciona la diócesis y avanza por sus niveles."
              ...
              onChange={(value) => {
                ...
                setDraftField('current_service_entity_id', value)
                setDraftField('quick_entity_id', value)
              }}
            />
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  setDraftField('quick_office_configuration_id', event.target.value)
                }}
              ...
              <div className="card compact-section">
                ...
                <div className="admin-form-fields-grid">
                  ...
                  <label>
                    Fecha de inicio
                    <input
                      name="quick_start_date"
                      type="date"
                      defaultValue={fieldValue('quick_start_date')}
                    />
                  </label>
                </div>
                <label>
                  Notas públicas del cargo
                  <textarea
                    name="quick_notes_public"
                    defaultValue={fieldValue('quick_notes_public')}
                  />
                </label>
              </div>
            ) : null}
            ...
            <div className="card compact-section">
              <label className="role-pill">
                <input
                  name="not_identified_fields"
                  type="checkbox"
                  value="current_service_entity_id"
                  defaultChecked={notIdentifiedFields.includes('current_service_entity_id')}
                />
                <span>Entidad de servicio no identificada</span>
              </label>
              <label>
                Notas internas
                <textarea name="notes_internal" defaultValue={fieldValue('notes_internal')} />
              </label>
            </div>
          </section>
          ...
          <section hidden={step !== 4}>
            ...
            <p className="meta">
              Después de crear la ficha podrás editar sus secciones y registrar eventos por separado.
            </p>

            <div className="admin-review-grid" aria-label="Resumen del registro sacerdotal">
              <article className="card compact-section">
                <span>Persona</span>
                <strong>{namePreview || 'Sin nombre visible'}</strong>
                <small>
                  {mode === 'existing-deacon'
                    ? 'Se completará la ficha existente'
              ...
              <article className="card compact-section">
                ...
                <small>
                  {priestType === 'religious'
                    ? fieldValue('religious_institute_name') || 'Instituto pendiente'
                    : incardination?.direct_entity_name || 'Incardinación pendiente'}
                </small>
                <button type="button" onClick={() => setStep(2)}>Cambiar</button>
              </article>
              <article className="card compact-section">
                <span>Ordenación</span>
                <strong>
                  {fieldValue('priestly_ordination_date') || 'Fecha no identificada'}
                </strong>
                <small>Estado: {fieldValue('canonical_status', 'active')}</small>
                <button type="button" onClick={() => setStep(2)}>Cambiar</button>
              </article>
              <article className="card compact-section">
                <span>Servicio</span>
                ...
                <small>
                  {filteredOfficeConfigs.find((office) => office.id === quickOfficeConfigId)
                    ?.display_name ?? 'Sin cargo actual'}
                </small>
          ...
          >
            ...
            {step < wizardSteps.length - 1 ? (
              ...
              <button
                ...
                type="submit"
              >
                {saving ? 'Guardando...' : 'Crear sacerdote'}
              </button>
            )}
          </div>
        </form>
        ...
        <SmartContextPanel title="Ayuda contextual">
          {completionItems.map((item) => (
            <div className="admin-context-block" key={item.label}>
              <span>{item.label}</span>
              <strong>{item.complete ? 'Completo' : 'Pendiente'}</strong>
            </div>
          ))}
          <div className="admin-context-block">
            <span>Entidad del cargo</span>
            <strong>{quickEntity?.direct_entity_name ?? 'No seleccionada'}</strong>
          </div>
          <div className="admin-context-block">
            <span>Borrador</span>
            <strong>Guardado automáticamente</strong>
          </div>
        </SmartContextPanel>
      </div>
    </main>
