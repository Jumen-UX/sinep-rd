...
function ObjectDetails({ value, omit = [] }: { value: unknown; omit?: string[] }) {
...
function CanonicalProposalReview({ original, proposed }: { original: JsonObject; proposed: JsonObject }) {
  ...
  return (
    <>
      ...
      <Section eyebrow="Sacramento" title="Operaciones sobre los grados del Orden">
        {ordinations.length === 0 ? <div className="empty-state">No se proponen cambios de ordenación.</div> : (
          <div className="grid admin-modules">
            {ordinations.map((ordination, index) => (
              <OperationCard key={`${String(ordination.degree)}-${index}`} title={valueText(ordination.degree)} value={ordination} />
            ))}
          </div>
        )}
      </Section>
      <Section eyebrow="Dimensiones canónicas" title="Vigencias propuestas">
        <div className="grid admin-modules">
          {operations.map(([title, operation]) => (
            isObject(operation) && operation.mode !== 'keep' ? <OperationCard key={title} title={title} value={operation} /> : null
          ))}
          {dignities.map((dignity, index) => (
            <OperationCard key={`${String(dignity.dignity_type)}-${index}`} title="Dignidad eclesiástica" value={dignity} />
          ))}
        </div>
        {!operations.some(([, operation]) => isObject(operation) && operation.mode !== 'keep') && dignities.length === 0
...
export default function ChangeRequestReviewPage() {
  ...
  async function loadItem() {
    ...
    try {
      ...
      setItem(await loadChangeRequestDetail(supabase, requestId))
    } catch (loadError) {
  ...
  useEffect(() => {
    if (requestId) void loadItem()
    // loadItem depends on the stable route id and Supabase client.
  ...
  async function review(decision: 'approved' | 'rejected') {
    ...
    setNotice(null)
    try {
      await reviewPersonChangeRequest(supabase, {
        requestId: item.id,
        decision,
  ...
  if (!item) return <main className="container dashboard-page"><div className="detail-backlink"><Link href="/admin/solicitudes">← Volver a solicitudes</Link></div><div className="empty-state">No tienes acceso a esta solicitud o no existe.</div></main>

  const isCanonicalPersonProposal = item.proposed_data?.proposal_kind === 'canonical_person'
  const canDecide = item.can_review && item.target_table === 'persons' && ['pending_review', 'needs_changes'].includes(item.status ?? '')
  ...
  return (
    <main className="container dashboard-page admin-config-page">
      ...
      {isCanonicalPersonProposal && item.original_data && item.proposed_data ? (
        ...
        <Section eyebrow="Comparación" title="Datos originales vs. propuesta"><GenericComparison item={item} /></Section>
      )}
      {canDecide && (
        <Section eyebrow="Decisión" title="Aprobar o rechazar" description="La aprobación aplica todas las dimensiones canónicas en una sola transacción y deja auditoría.">
          <div className="admin-actions">
            <button className="button button-primary" disabled={saving} type="button" onClick={() => void review('approved')}>
          ...
          <form className="auth-form access-form" onSubmit={handleReject}>
            ...
            <button className="button button-secondary" disabled={saving} type="submit">{saving ? 'Procesando...' : 'Rechazar'}</button>
          </form>
        </Section>
      )}
    </main>
