...
function EyeIcon({ hidden }: { hidden: boolean }) {
  return hidden ? (
    <svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18"><path d="M3 3l18 18M10.6 10.7a2 2 0 002.7 2.7M9.9 4.2A10.8 10.8 0 0112 4c5.4 0 9 5 9 5a16.8 16.8 0 01-3.1 3.6M6.2 6.2C4.1 7.6 3 9 3 9s3.6 5 9 5c1 0 1.9-.2 2.7-.5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>
  ) : (
    <svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18"><path d="M3 12s3.6-5 9-5 9 5 9 5-3.6 5-9 5-9-5-9-5z" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /><circle cx="12" cy="12" r="2.4" fill="none" stroke="currentColor" strokeWidth="1.8" /></svg>
  )
}

function LockIcon() {
  return <svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18"><rect x="5" y="10" width="14" height="10" rx="2" fill="none" stroke="currentColor" strokeWidth="1.8" /><path d="M8 10V7a4 4 0 018 0v3" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg>
}
      ...
      <section className={`${styles.panel} ${styles.passwordPanel} ${lowerStyles.progressiveSecurity}`} aria-labelledby="password-title">
        <div className={styles.passwordSummary}>
          ...
          <div>
            ...
            <h2 id="password-title">Protege tu acceso</h2>
            <p>{passwordExpanded ? 'Define una contraseña nueva que cumpla todos los requisitos.' : 'Actualiza tu contraseña solo cuando lo necesites. El formulario permanece oculto para reducir distracciones y proteger tus datos.'}</p>
          </div>
          {!passwordExpanded ? (
            <button aria-controls="password-change-form" aria-expanded="false" className={styles.primaryButton} onClick={openPasswordForm} type="button">Cambiar contraseña</button>
          ) : null}
        </div>

        {passwordExpanded ? (
          <form className={`${styles.form} ${styles.expandedForm}`} id="password-change-form" onSubmit={changePassword}>
            <div className={styles.expandedHeader}>
              <div><strong>Cambiar contraseña</strong><p>Los campos se limpiarán al cancelar o completar el cambio.</p></div>
              <button className={styles.cancelInlineButton} disabled={busy === 'password'} onClick={closePasswordForm} type="button">Cancelar</button>
            </div>

            <div className={styles.passwordField}>
              <label htmlFor="new-password">Nueva contraseña</label>
              <div className={styles.inputGroup}>
                <input ref={newPasswordRef} id="new-password" aria-describedby="password-strength password-requirements" autoComplete="new-password" minLength={MIN_PASSWORD_LENGTH} onChange={(event) => setPassword(event.target.value)} placeholder="Escribe una contraseña nueva" required type={showPassword ? 'text' : 'password'} value={password} />
