...
export default function StructureEntityPicker({
  ...
  useEffect(() => {
    if (!value || dioceses.length === 0) return

    const legacyMatch = nodes.find((node) => node.linked_ecclesiastical_entity_id === value)
    if (legacyMatch) return

    // Deja la diócesis intacta si todavía no sabemos dónde está el valor.
  }, [value, dioceses.length, nodes])
