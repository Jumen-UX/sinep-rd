  ...
  return (
    <div className="action-editor">
      ...
      <div className="editor-grid">
        <label className="field">Nodo sujeto
          <select value={subjectNodeId} onChange={(event) => setSubjectNodeId(event.target.value)}><option value="">Seleccionar nodo</option>{(options?.nodes ?? []).map((node) => <option key={node.id} value={node.id}>{node.name} · {node.level_name}</option>)}</select>
        </label>
        <label className="field">Nodo destino
          <select value={targetNodeId} onChange={(event) => setTargetNodeId(event.target.value)}><option value="">Seleccionar destino</option>{(options?.nodes ?? []).map((node) => <option key={node.id} value={node.id}>{node.name} · {node.level_name}</option>)}</select>
        </label>
        <label className="field">Padre anterior
          <select value={parentBeforeNodeId} onChange={(event) => setParentBeforeNodeId(event.target.value)}><option value="">Seleccionar padre anterior</option>{(options?.nodes ?? []).map((node) => <option key={node.id} value={node.id}>{node.name} · {node.level_name}</option>)}</select>
        </label>
        <label className="field">Padre posterior
          <select value={parentAfterNodeId} onChange={(event) => setParentAfterNodeId(event.target.value)}><option value="">Seleccionar padre posterior</option>{(options?.nodes ?? []).map((node) => <option key={node.id} value={node.id}>{node.name} · {node.level_name}</option>)}</select>
        </label>
        <label className="field">Nivel anterior
          <select value={levelBeforeId} onChange={(event) => setLevelBeforeId(event.target.value)}><option value="">Seleccionar nivel anterior</option>{(options?.levels ?? []).map((level) => <option key={level.id} value={level.id}>{level.name}</option>)}</select>
        </label>
        <label className="field">Nivel posterior
          <select value={levelAfterId} onChange={(event) => setLevelAfterId(event.target.value)}><option value="">Seleccionar nivel posterior</option>{(options?.levels ?? []).map((level) => <option key={level.id} value={level.id}>{level.name}</option>)}</select>
        </label>
...
export default function StructuralApplicationPlanPage() {
  ...
  async function loadPlan() {
    ...
    const [planRes, optionsRes] = await Promise.all([supabase.rpc('get_structural_application_plan', { p_event_id: eventId }), supabase.rpc('get_structural_action_editor_options', { p_event_id: eventId })])
  ...
  async function generatePlan() { setSaving(true); setError(null); const { data, error: generateError } = await supabase.rpc('admin_generate_structural_application_plan', { payload: { event_id: eventId } }); if (generateError) { setError(generateError.message); setSaving(false); return } setPlan((data ?? null) as PlanResponse | null); setSaving(false) }
  async function updateAction(actionId: string, status: ActionStatus) { setSaving(true); setError(null); const { data, error: updateError } = await supabase.rpc('admin_update_structural_event_action', { payload: { action_id: actionId, status } }); if (updateError) { setError(updateError.message); setSaving(false); return } setPlan((data ?? null) as PlanResponse | null); setSaving(false) }
  async function configureAction(payload: Record<string, unknown>) { setSaving(true); setError(null); const { data, error: configureError } = await supabase.rpc('admin_configure_structural_event_action', { payload }); if (configureError) { setError(configureError.message); setSaving(false); return } setPlan((data ?? null) as PlanResponse | null); setSaving(false) }
  ...
  return (
    <main className="container dashboard-page structural-plan-page">
      ...
      <section className="dashboard-hero card plan-hero"><div><p className="eyebrow">Fase 2 · plan de aplicación estructural</p><h1>{event.title}</h1><p className="lead">Convierte el impacto estructural en acciones revisables y permite completar datos faltantes. Todavía no crea, mueve, cierra ni renombra nodos.</p><div className="button-row"><button className="button button-primary" disabled={!summary.can_generate_plan || saving} onClick={generatePlan} type="button">{saving ? 'Procesando...' : 'Generar / regenerar plan'}</button><Link className="button button-secondary" href={`/admin/estructura/eventos/${eventId}`}>Vista de impacto</Link><Link className="button button-secondary" href="/admin/estructura/eventos">Registro estructural</Link></div></div><div className="plan-summary"><span className={`mini-badge ${statusClass(event.status)}`}>{statusLabel(event.status)}</span><strong>{event.event_type_name}</strong><span className="meta">{event.template_name ?? 'Sin estructura'} · {formatDate(event.effective_date)}</span></div></section>
      {error && <div className="error-box">{error}</div>}
      <section className="metric-grid"><div className="plan-card"><strong>{summary.action_count}</strong><span className="meta">acciones</span></div><div className="plan-card"><strong>{summary.state_changing_count}</strong><span className="meta">cambiarían estructura</span></div><div className="plan-card"><strong>{summary.blocker_count}</strong><span className="meta">bloqueos de impacto</span></div><div className="plan-card"><strong>{summary.warning_count}</strong><span className="meta">advertencias</span></div></section>
