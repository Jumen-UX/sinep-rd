...
const pageStyles = `
  ...
  .events-toolbar,.events-tabs,.events-layout,.events-metrics,.events-timeline,.facets-grid,.derived-list,.impact-list,.event-detail-grid{display:grid;gap:14px}
...
export default function AdminEventosPage() {
  ...
  return (
    <main className="container dashboard-page events-page">
      ...
      <section className="events-layout"><div className="card dashboard-section"><div className="section-heading"><div><p className="eyebrow">Línea de eventos</p><h2>{filteredEvents.length} resultados</h2><p className="meta">Selecciona un evento para ver qué páginas alimenta y qué impacto tendría al aplicarse.</p></div></div><div className="events-timeline">{filteredEvents.length === 0 && <div className="empty-state">No hay eventos con esos filtros.</div>}{filteredEvents.map((event) => <button className={`event-card-button ${selectedEvent && eventKey(event) === eventKey(selectedEvent) ? 'active' : ''}`} key={eventKey(event)} onClick={() => setSelectedEventKey(eventKey(event))} type="button"><div className="event-card-main"><div className="event-date-box"><strong>{dayValue(event.event_date)}</strong><span>{monthLabel(event.event_month).slice(0, 3)}</span><span>{event.event_year ?? '—'}</span></div><div className="event-info"><h3>{event.title}</h3><p className="meta">{formatDate(event.event_date)} · {event.event_type_name ?? event.event_type_key ?? 'Tipo no definido'}</p><div className="badge-row"><span className="mini-badge">{loadModeLabel(event.load_mode)}</span><span className={`mini-badge ${isEvidencePending(event) ? 'warning' : 'success'}`}>{evidenceLabel(event.evidence_status)}</span>{event.workflow_status === 'pending_review' && <span className="mini-badge warning">Pendiente de revisión</span>}{event.related_entity_name && <span className="mini-badge">{event.related_entity_name}</span>}{event.source_name && <span className="mini-badge">Fuente: {event.source_name}</span>}</div></div></div></button>)}</div></div><aside className="facets-grid"><EventDetailPanel event={selectedEvent} /><div className="facet-card highlight"><strong>Asistentes</strong><Link className="button button-primary" href="/admin/eventos/nuevo">Preparar evento</Link></div></aside></section>
    </main>
  )
}
