...
const steps: Array<{ key: AssistantStep; title: string; description: string }> = [
  { key: 'modo', title: 'Modo', description: 'Define si es historia, evento nuevo o foto inicial.' },
  { key: 'hecho', title: 'Hecho', description: 'Fecha, tipo y título del evento.' },
  { key: 'afectados', title: 'Afectados', description: 'Entidad principal y relación del evento.' },
  { key: 'fuente', title: 'Fuente', description: 'Documento, evidencia y nivel de confianza.' },
  { key: 'impacto', title: 'Impacto', description: 'Vista previa antes de guardar.' },
]
...
function derivedPages({ title, eventDate, entityName, eventTypeName, sourceName }: { title: string; eventDate: string; entityName: string; eventTypeName: string; sourceName: string }) {
  ...
  if (title && !pages.includes(title)) pages.push('Búsqueda y línea histórica')
  return pages
}
...
export default function NuevoEventoPage() {
  ...
  const pages = derivedPages({ title: computedTitle, eventDate: eventDate || effectiveDate, entityName, eventTypeName, sourceName })
  ...
  return (
    <main className="container dashboard-page event-assistant-page">
      ...
      <section className="dashboard-hero card assistant-hero">
        <div>
          <p className="eyebrow">Asistente visual · sin escritura final</p>
          <h1>Preparar evento</h1>
          <p className="lead">Este asistente permite simular la carga histórica o un evento nuevo antes de guardarlo. El objetivo es validar datos, evidencia, páginas derivadas e impacto.</p>
        </div>
        <div className="assistant-summary-card">
          <span className="mini-badge">{modeLabel(loadMode)}</span>
          <strong>{isReadyForPreview ? 'Listo para revisar' : 'Borrador visual'}</strong>
          <span className="meta">Todavía no escribe en la base; primero validamos el flujo.</span>
        </div>
      </section>
      ...
      <section className="assistant-layout">
        <div className="card dashboard-section">
            ...
            <>
              <div className="section-heading">
                <div>
                  ...
                  <h2>Qué estás registrando</h2>
                  <p className="meta">La historia pasada, un cambio nuevo y la foto inicial vigente se tratan distinto, pero alimentan el mismo registro histórico-documental.</p>
                </div>
              </div>
              <div className="mode-grid">
                {loadModes.map((mode) => (
                  <button className={`mode-card ${loadMode === mode.key ? 'active' : ''}`} key={mode.key} onClick={() => setLoadMode(mode.key)} type="button">
                    ...
                    <span className="meta">{mode.description}</span>
                  </button>
                ))}
              </div>
            </>
            ...
            <>
              <div className="section-heading">
                <div>
                  ...
                  <p className="meta">El evento necesita tipo, fecha, título y una descripción suficiente para identificarlo.</p>
                </div>
              </div>
              <div className="form-grid">
                ...
                <label>Título
                  <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder={buildTitle(eventTypeName, entityName) || 'Ej. Erección de la Diócesis de X'} />
                </label>
                <label className="full-width">Descripción
                  <textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Resumen del hecho, contexto y resultado esperado." />
                </label>
              </div>
            </>
          )}
            ...
            <>
              <div className="section-heading">
                <div>
                  ...
                  <h2>Entidad afectada</h2>
                  <p className="meta">Cada evento debe apuntar a una entidad principal. Después agregaremos múltiples participantes: entidad madre, nueva entidad, ordinario, provincia, territorio, etc.</p>
                </div>
              </div>
              <div className="form-grid">
                <label>Entidad principal
                  <select value={entityId} onChange={(event) => setEntityId(event.target.value)}>
                    {entities.map((entity) => <option key={entity.id} value={entity.id}>{entity.name} · {entity.entity_types?.name ?? 'Entidad'}</option>)}
                  </select>
                </label>
                <label>Rol en el evento
                  <select value={entityRole} onChange={(event) => setEntityRole(event.target.value)}>
                    <option value="affected_jurisdiction">Jurisdicción afectada</option>
                    <option value="created_entity">Entidad creada</option>
                    <option value="mother_jurisdiction">Jurisdicción madre</option>
                    <option value="metropolitan_see">Sede metropolitana</option>
                    <option value="suffragan_jurisdiction">Sufragánea</option>
                    <option value="authority">Autoridad</option>
                    <option value="ordinary">Ordinario</option>
                  </select>
                </label>
                <label className="full-width">Notas sobre afectados
                  <textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Ej. fue desmembrada de..., pasa a ser sufragánea de..., afecta territorio de..." />
                </label>
              </div>
            </>
          )}
            ...
            <>
              <div className="section-heading">
                <div>
                  ...
                  <p className="meta">El sistema puede cargar historia incompleta, pero debe decir claramente si está confirmada, importada, secundaria o pendiente.</p>
                </div>
              </div>
              <div className="form-grid">
                <label>Estado de evidencia
                  <select value={evidenceStatus} onChange={(event) => setEvidenceStatus(event.target.value)}>
                    {evidenceOptions.map((item) => <option key={item.key} value={item.key}>{item.name}</option>)}
                  </select>
                </label>
                <label>Nombre de la fuente
                  <input value={sourceName} onChange={(event) => setSourceName(event.target.value)} placeholder="Ej. Boletín Santa Sede, AAS, Catholic-Hierarchy, Directorio..." />
                </label>
                <label className="full-width">URL / referencia
                  <input value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://... o referencia interna" />
                </label>
              </div>
            </>
          )}
            ...
            <>
              <div className="section-heading">
                <div>
                  ...
                  <p className="meta">Antes de guardar, el sistema muestra qué páginas alimenta y qué faltaría para aplicar el evento.</p>
                </div>
              </div>
              <div className="impact-grid">
                ...
                <div className="impact-card"><strong>Estado</strong><span className="meta">{isReadyForPreview ? 'Puede pasar a revisión visual.' : 'Faltan datos mínimos.'}</span></div>
              </div>
            </>
          )}
          ...
          <div className="button-row">
            ...
            <button className="button button-secondary" disabled type="button">Guardar pendiente</button>
          </div>
        </div>
        ...
        <aside className="card dashboard-section">
          <div className="section-heading">
            <div>
              ...
              <h2>{computedTitle || 'Evento sin título'}</h2>
              <p className="meta">Esta ficha se construye con lo seleccionado. Todavía no escribe datos.</p>
            </div>
          </div>
          ...
          <div className="derived-card highlight">
            ...
            <span className="meta">Estas vistas se podrán generar desde este evento.</span>
          </div>
          <div className="derived-list">
            {pages.map((page) => <div className="derived-card" key={page}><span className="meta">{page}</span></div>)}
          </div>

          <div className="impact-card highlight">
            <strong>Faltante para guardar</strong>
            <span className="meta">Activar RPC segura para crear evento pendiente, participantes, fuente y vista previa real de relaciones.</span>
          </div>
        </aside>
      </section>
    </main>
