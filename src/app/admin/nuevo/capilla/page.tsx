...
const steps = ['Identidad', 'Dependencia', 'Ubicación', 'Completitud', 'Revisión']
const optionalFields: MissingField[] = [
  ...
  { key: 'erected_at', label: 'Fecha de creación' },
  { key: 'territory_summary', label: 'Comunidad o sector' },
]
...
export default function NuevaCapillaPage() {
  ...
  const chapelType = entityTypes.find((item) => item.key === 'chapel')
  ...
  async function loadData() {
    ...
    const [typeRes, parentRes] = await Promise.all([
      supabase.from('entity_types').select('id,key,name').in('key', ['archdiocese', 'diocese', 'military_ordinariate', 'vicariate', 'pastoral_zone', 'deanery', 'pastoral_region', 'parish', 'quasi_parish']).order('default_level_order'),
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!chapelType) {
      setError('No se encontró el tipo de entidad Capilla en la base de datos.')
      setSaving(false)
      return
    }
    ...
    if (parentId && saved?.id) {
      const relRes = await supabase.from('entity_relationships').insert({
        ...
        is_current: true,
        status: 'active',
        notes: 'Relación creada desde asistente de nueva capilla.',
        created_by: userData.user?.id ?? null,
      })
      if (relRes.error) {
    ...
    if (saved?.id && notIdentifiedFields.length > 0) {
      const rows = notIdentifiedFields.map((field) => ({
        ...
        field_name: field,
        status: 'unknown',
        notes: 'Marcado como no identificado desde el asistente de nueva capilla.',
        created_by: userData.user?.id ?? null,
      }))
    ...
    <main className="container dashboard-page admin-config-page">
      ...
      <section className="dashboard-hero card">
        <div>
          ...
          <h1>Nueva capilla</h1>
          <p className="lead">Crea una capilla y vincúlala a su parroquia, zona o diócesis. El sistema mantendrá separada su dependencia territorial.</p>
        </div>
      </section>
      ...
      <form className="admin-form admin-config-form card dashboard-section" onSubmit={handleSubmit}>
        ...
        {step === 3 && (
          <section>
            ...
            <label>Fecha de creación<input name="erected_at" type="date" /></label>
            ...
            <div className="card compact-section">
              ...
              <p className="meta">Marca los datos que no se pudieron identificar. No generarán alertas de completitud.</p>
        ...
        {step === 4 && (
          <section>
            ...
            <p className="lead">Guarda la capilla. Luego podrás completar responsable, fotos, horarios de celebración y documentos desde asistentes posteriores.</p>
          </section>
        )}
